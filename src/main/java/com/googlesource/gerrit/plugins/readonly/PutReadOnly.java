...
import com.googlesource.gerrit.plugins.readonly.PutReadOnly.Input;
import java.io.IOException;

@RequiresAnyCapability({ADMINISTRATE_SERVER, MAINTAIN_SERVER})
@Singleton
class PutReadOnly implements RestModifyView<ConfigResource, Input> {
  static class Input {}

  private final ReadOnlyState state;

  @Inject
  PutReadOnly(ReadOnlyState state) {
    this.state = state;
  }
